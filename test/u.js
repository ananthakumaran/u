...
var oneOf = jsc.nearray(jsc.json).smap(
    (array) => {
...
var generateObject = jsc.generator.recursive(
    ...
    function (gen) {
        return jsc.generator.oneof([
            jsc.generator.dict(gen).map(wrap),
            jsc.generator.nearray(gen).map(wrapTuple),
            gen.map(wrapArray),
        ]);
    },
);
...
var shrinkObject = jsc.shrink.bless((value) => {
    ...
    if (_.isArray(spec)) {
        ...
        switch (type) {
            case "oneOf":
                return oneOf.shrink(value);
            case "boolean":
                return boolean.shrink(value);
            case "tuple":
                return jsc.shrink
                    .nearray(shrinkObject)(unwrapTuple(value))
                    .map(wrapTuple);
            case "array":
...
var shrinkDictObject = (() => {
    ...
    return arrayShrink.smap(
        jsc.utils.pairArrayToDict,
        jsc.utils.dictToPairArray,
    );
})();
        ...
        jsc.assert(
            jsc.forall(generator, (x) => {
                ...
                if (!_.isEqual(value, decoded)) {
                    ...
                    console.log(
                        "decoded",
                        util.inspect(decoded, { depth: null }),
                    );
                }
...
describe("u", () => {
    describe("primitives", () => {
        ...
        it(
            "tuple",
            validateExample(["tuple", ["integer"], ["boolean"]], [0, true]),
        );
        it("array", () => {
    ...
    describe("version", function () {
        it("picks the correct version", function () {
            ...
            assert.deepEqual(decode(coders, encode(version1, { a: 10 })), {
                a: 10,
            });
            assert.deepEqual(decode(coders, encode(version2, { a: true })), {
                a: true,
            });
            assert.deepEqual(decode(coders, encode(version3, { d: "he" })), {
                d: "he",
            });
        });
    });
    ...
    describe("example", () => {
        it("works", () => {
            ...
            assert.deepEqual(decode([v1], encodedv1), {
                lookingFor: "bride",
                age: [25, 30],
                religion: "Hindu",
                motherTongue: "Bengali",
            ...
            assert.deepEqual(decode([v1, v2], encodedv1), {
                lookingFor: "bride",
                age: [25, 30],
                religion: "Hindu",
                motherTongue: "Bengali",
                onlyProfileWithPhoto: true,
                maritialStatus: "Doesn't Matter",
            });
            var encodedv2 = encode(v2, {
                lookingFor: "bride",
                age: [25, 30],
                religion: "Hindu",
                motherTongue: "Bengali",
            ...
            assert.deepEqual(decode([v1, v2], encodedv2), {
                lookingFor: "bride",
                age: [25, 30],
                religion: "Hindu",
                motherTongue: "Bengali",
